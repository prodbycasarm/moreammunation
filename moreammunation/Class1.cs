...
namespace moreammunation
{
    ...
    public class Main : Script
    {
        ...
        private Vehicle robberyHeistVehicle = null;
        ...
        private WeaponHash GetRandomWeapon()
        {
            ...
            if (roll < 80)      // 40%
                return RiflesValues.ElementAt(_rng.Next(RiflesValues.Count)).Key;
            else if (roll < 55) // 15%
                return SMGsValues.ElementAt(_rng.Next(SMGsValues.Count)).Key;
            else if (roll < 70) // 15%
                return ShotgunsValues.ElementAt(_rng.Next(ShotgunsValues.Count)).Key;
            else if (roll < 60) // 20%
                return HandgunsValues.ElementAt(_rng.Next(HandgunsValues.Count)).Key;
            else                // 10%
                return MachineGunsValues.ElementAt(_rng.Next(MachineGunsValues.Count)).Key;
        }


        // Config and key settings
        ScriptSettings config;
        Keys enable = Keys.E; // default key for armory
        Keys heistKey = Keys.Q; // default key for heist
        ...
        private void CleanupExistingArmoryEntities()
        {
            ...
            // Delete active heist vehicle and cleanup its blip
            if (robberyHeistVehicle != null && robberyHeistVehicle.Exists())
            {
                if (heistBlip != null && heistBlip.Exists())
                    heistBlip.Delete();

                robberyHeistVehicle.MarkAsNoLongerNeeded();
                robberyHeistVehicle.Delete();
                robberyHeistVehicle = null;
            }
        ...
        private void LoadarmoryZonePositions()
        {
            ...
            while (true)
            {
                ...
                string blipSpriteString = config.GetValue<string>($"ArmoryZone{zoneIndex}", "BlipSprite", "Ammu-Nation +");
            ...
            GTA.UI.Notification.Show(
                ...
                "Agent 16",
                "Ammu-Nation +",
                "~w~Hey, Agent 16 here.~n~I’ve got some heists that might interest you. Just give me a call !~n~There’s also new Ammunation trucks on the map you can rob for some extra cash.",
                true,
                false
            );
            ...
            foreach (var zone in armoryZones)
            {
                ...
                if (zone.SpawnVehicle)
                {
                    ...
                    if (vehicleModel.IsInCdImage && vehicleModel.IsValid)
                    {
                        ...
                        vehicle = World.CreateVehicle(vehicleModel, zone.Position + new Vector3(0f, 0f, 0f));
        ...
        private void SpawnGroundTroops(HeistLocation loc)
        {
            if (loc.GroundTroops == null || loc.GroundTroopPositions == null)
                return;

            for (int i = 0; i < loc.GroundTroops.Count; i++)
            {
                string pedModelName = loc.GroundTroops[i];
                Vector3 spawnPos = loc.GroundTroopPositions[i];
                ...
                ped.RelationshipGroup = ARMORY_GUARDS;

                ped.Weapons.Give(GetRandomWeapon(), 9999, true, true);
        ...
        private void LoadHeistLocations()
        {
            ...
            foreach (string file in files)
            {
                try
                {
                    ...
                    // 2. SNAP TO ROAD (Ensures mission marker is on the street)
                    Vector3 snappedPos = World.GetNextPositionOnStreet(rawPos, true);

                    HeistLocation loc = new HeistLocation
                    {
                        Name = node["Name"].InnerText,
                        Position = snappedPos, // Using the fixed road position
                        Radius = float.Parse(node["Radius"].InnerText),
                        Description = node["Description"].InnerText,
                        Reward = (int)float.Parse(node["Reward"].InnerText)
                    };
                    ...
                    // -------- TARGET VEHICLE --------
                    XmlNode targetVehicleNode = node.SelectSingleNode("TargetVehicle");
                    if (targetVehicleNode != null)
                    {
                        loc.VehicleModel = targetVehicleNode["TargetModelName"]?.InnerText;

                        // Parse Peds: first = driver, rest = passengers
                        XmlNode pedsNode = targetVehicleNode.SelectSingleNode("Peds");
                        if (pedsNode != null)
                        {
                            var pedNodes = pedsNode.SelectNodes("Ped");
                            if (pedNodes.Count > 0)
                            {
                                loc.VehicleDriverModel = pedNodes[0].InnerText.Trim(); // First ped = driver

                                for (int i = 1; i < pedNodes.Count; i++)
                                {
                                    if (!string.IsNullOrWhiteSpace(pedNodes[i].InnerText))
                                        loc.TargetPedModels.Add(pedNodes[i].InnerText.Trim());
                                }
                            }
                        }

                        loc.VehiclePosition = new Vector3(
                            float.Parse(targetVehicleNode["PositionX"]?.InnerText ?? "0"),
                            float.Parse(targetVehicleNode["PositionY"]?.InnerText ?? "0"),
                            float.Parse(targetVehicleNode["PositionZ"]?.InnerText ?? "0")
                        );

                        loc.TargetRotation = float.Parse(targetVehicleNode["Rotation"]?.InnerText ?? "0");
                    }

                    // -------- AREA VEHICLES --------
                    XmlNode areaVehiclesNode = node.SelectSingleNode("AreaVehicles");
                    if (areaVehiclesNode != null)
                    {
                        foreach (XmlNode vehNode in areaVehiclesNode.SelectNodes("Vehicle"))
                        {
                            AreaVehicle av = new AreaVehicle
                            {
                                ModelName = vehNode["ModelName"]?.InnerText,
                                Position = new Vector3(
                                    float.Parse(vehNode["PositionX"]?.InnerText ?? "0"),
                                    float.Parse(vehNode["PositionY"]?.InnerText ?? "0"),
                                    float.Parse(vehNode["PositionZ"]?.InnerText ?? "0")
                                ),
                                Rotation = float.Parse(vehNode["Rotation"]?.InnerText ?? "0")
                            };

                            // Load driver models
                            XmlNode driverModelsNode = vehNode.SelectSingleNode("DriverModels");
                            if (driverModelsNode != null)
                            {
                                foreach (XmlNode pedNode in driverModelsNode.SelectNodes("Ped"))
                                {
                                    if (!string.IsNullOrWhiteSpace(pedNode.InnerText))
                                        av.PedModels.Add(pedNode.InnerText.Trim());
                                }
                            }

                            loc.AreaVehicles.Add(av);
                        }



                        // -------- GROUND TROOPS --------
                        XmlNode groundTroopsNode = node.SelectSingleNode("GroundTroops");
                        if (groundTroopsNode != null)
                        {
                            foreach (XmlNode pedNode in groundTroopsNode.SelectNodes("Ped"))
                            {
                                if (!string.IsNullOrWhiteSpace(pedNode.InnerText))
                                {
                                    // Instead of using XML coords, place them 5.0m to 10.0m around the vehicle
                                    // 'Around' is a common helper; if not present, use the manual math below
                                    Vector3 troopPos = loc.VehiclePosition.Around(8.0f);

                                    // Snap to ground so they don't spawn in the air or underground
                                    float groundZ = World.GetGroundHeight(troopPos);
                                    loc.GroundTroopPositions.Add(new Vector3(troopPos.X, troopPos.Y, groundZ));
                                    loc.GroundTroops.Add(pedNode.InnerText.Trim());
                                }
                            }
                        }
        ...
        private void ExitWeaponDeliveryHeist()
        {
            ...
            // Instead of immediately deleting the active heist vehicle, queue it to respawn after 5 seconds.
            // Prefer using the original zone/model if available (vehicleZoneMapping or activeHeistLocation).
            if (activeHeistVehicle != null && activeHeistVehicle.Exists())
            {
                ArmoryZone respawnZone = null;

                // If this vehicle came from a zone spawn, reuse that zone
                if (vehicleZoneMapping.TryGetValue(activeHeistVehicle, out ArmoryZone mappedZone))
                {
                    respawnZone = mappedZone;
                }
                // If it's a contact heist vehicle, try to use activeHeistLocation info
                else if (activeHeistLocation != null && !string.IsNullOrEmpty(activeHeistLocation.VehicleModel))
                {
                    respawnZone = new ArmoryZone
                    {
                        Position = activeHeistLocation.VehiclePosition,
                        VehicleName = activeHeistLocation.VehicleModel,
                        BlipName = activeHeistLocation.Name,
                        BlipSprite = "Ammu-Nation +",
                        BlipColor = "BlueLight"
                    };
                }
                ...
                if (respawnZone != null)
                {
                    // Queue a respawn (OnTick will actually delete the old vehicle at RespawnTime and spawn a new one)
                    vehiclesToRespawn.Add(new VehicleRespawn
                    {
                        Zone = respawnZone,
                        Vehicle = activeHeistVehicle,
                        RespawnTime = DateTime.Now.AddSeconds(5)
                    });

                    // Also queue deletion of the old entity after the same delay to avoid duplicates
                    vehiclesToDelete.Add((activeHeistVehicle, DateTime.Now.AddSeconds(5)));
                }
                else
                {
                    // Fallback: immediate cleanup if we cannot determine a respawn target
                    activeHeistVehicle.MarkAsNoLongerNeeded();
                    activeHeistVehicle.Delete();
                }
        ...
        private bool heistAIActivated = false;
        private const float AI_ACTIVATION_DISTANCE = 900f;
        private Ped mainHeistDriver;
        private bool driverPanicTriggered = false;
        private Dictionary<Ped, Blip> pedBlips = new Dictionary<Ped, Blip>();
        private int currentHeistIndex = 0;
        private RelationshipGroup ARMORY_GUARDS;
        private void SetupRelationships()
        {
            ARMORY_GUARDS = World.AddRelationshipGroup("ARMORY_GUARDS");
        ...
        {
            SetupRelationships();
            heistAIActivated = false;

            if (heistLocations.Count == 0)
            {
                GTA.UI.Notification.Show("~r~No locations found.");
                return;
            }
            if (currentHeistIndex >= heistLocations.Count)
                currentHeistIndex = 0;
            /*
             
            // For sequential mission cycling (optional, can switch to random)

            activeHeistLocation = heistLocations[currentHeistIndex];
            currentHeistIndex++;


            */
            // For random heist location */

            Random rnd = new Random();
            activeHeistLocation = heistLocations[_rng.Next(heistLocations.Count)];

            // --- Target vehicle ---
            Model vehicleModel = new Model(activeHeistLocation.VehicleModel);
            vehicleModel.Request();
            int vehicleTimeout = 0;
            while (!vehicleModel.IsLoaded && vehicleTimeout < 100)
            {
                Script.Yield();
                vehicleTimeout++;
            }
            ...
            {
                GTA.UI.Notification.Show($"~r~Failed to load target vehicle model: {activeHeistLocation.VehicleModel}");
                return;
            }

            activeHeistVehicle = World.CreateVehicle(vehicleModel, activeHeistLocation.VehiclePosition, activeHeistLocation.TargetRotation);
            if (activeHeistVehicle == null || !activeHeistVehicle.Exists())
            {
                GTA.UI.Notification.Show($"~r~Failed to spawn target vehicle: {activeHeistLocation.VehicleModel}");
                return;
            }

            Function.Call(Hash.SET_ENTITY_AS_MISSION_ENTITY, activeHeistVehicle.Handle, true, true);
            Function.Call(Hash.SET_ENTITY_LOAD_COLLISION_FLAG, activeHeistVehicle.Handle, true);
            activeHeistVehicle.IsPersistent = true;
            activeHeistVehicle.LockStatus = VehicleLockStatus.PlayerCannotEnter;

            heistBlip = activeHeistVehicle.AddBlip();
            heistBlip.Sprite = BlipSprite.BountyHit;
            heistBlip.Color = BlipColor.Red;
            heistBlip.ShowRoute = true;

            // --- Target vehicle NPCs ---
            VehicleSeat[] targetSeats = { VehicleSeat.Driver, VehicleSeat.Passenger, VehicleSeat.LeftRear, VehicleSeat.RightRear };
            List<string> pedModels = new List<string>();
            if (!string.IsNullOrEmpty(activeHeistLocation.VehicleDriverModel))
                pedModels.Add(activeHeistLocation.VehicleDriverModel);
            if (activeHeistLocation.TargetPedModels != null)
                pedModels.AddRange(activeHeistLocation.TargetPedModels);

            // Spawn peds up to available seats
            int targetPedCount = Math.Min(pedModels.Count, targetSeats.Length);
            for (int i = 0; i < pedModels.Count && i < targetSeats.Length; i++)
            {
                string pedModelName = pedModels[i];
                if (string.IsNullOrEmpty(pedModelName))
                {
                    GTA.UI.Notification.Show($"~y~Skipped empty ped model at seat {i}");
                    continue;
                }
                ...
                pedModel.Request();
                int pedTimeout = 0;
                while (!pedModel.IsLoaded && pedTimeout < 100)
                {
                    Script.Yield();
                    pedTimeout++;
                }

                if (!pedModel.IsLoaded || !pedModel.IsInCdImage || !pedModel.IsValid)
                {
                    GTA.UI.Notification.Show($"~r~Failed to load ped model: {pedModelName}");
                    continue;
                }

                Ped ped = activeHeistVehicle.CreatePedOnSeat(targetSeats[i], pedModel);
                if (ped == null || !ped.Exists())
                {
                    GTA.UI.Notification.Show($"~r~Failed to spawn ped {pedModelName} on seat {targetSeats[i]}");
                    continue;
                }

                if (i == 0) mainHeistDriver = ped;

                npcContactPeds.Add(ped);
                Function.Call(Hash.SET_ENTITY_AS_MISSION_ENTITY, ped.Handle, true, true);
                Function.Call(Hash.SET_ENTITY_LOAD_COLLISION_FLAG, ped.Handle, true);
                ped.IsPersistent = true;
                ped.BlockPermanentEvents = false;
                WeaponHash weapon = GetRandomWeapon();
                ped.Weapons.Give(weapon, 9999, true, true);

                if (i > 0) ped.RelationshipGroup = ARMORY_GUARDS;
                ...
                if (i > 0)
                {
                    ...
                    pedBlips[ped] = pedBlip;
                }


            }

            // --- Area vehicles (unchanged) ---
            VehicleSeat[] seats =
            {
                VehicleSeat.Driver,
            ...
            {
                Model model = new Model(av.ModelName);
                model.Request();
                while (!model.IsLoaded) Script.Yield();

                Vehicle v = World.CreateVehicle(model, av.Position, av.Rotation);
                Function.Call(Hash.SET_ENTITY_AS_MISSION_ENTITY, v.Handle, true, true);

                v.IsPersistent = true;
                npcContactVehicles.Add(v);
                spawnedAreaVehicles.Add(v);
                int pedCount = Math.Min(av.PedModels.Count, seats.Length);
                for (int i = 0; i < av.PedModels.Count && i < seats.Length; i++)
                {
                    Model pedModel = new Model(av.PedModels[i]);
                    pedModel.Request();
                    while (!pedModel.IsLoaded) Script.Yield();

                    Ped ped = v.CreatePedOnSeat(seats[i], pedModel);
                    if (ped == null || !ped.Exists())
                        continue;
            ...
            SpawnGroundTroops(activeHeistLocation);
        }

        // --- Spawn and activate heist AI ---
        private void ActivateHeistAI()
        {
            if (activeHeistVehicle == null || !activeHeistVehicle.Exists()) return;
            if (mainHeistDriver == null || !mainHeistDriver.Exists()) return;
            ...
            foreach (var veh in spawnedAreaVehicles)
            {
                ...
                {
                    // fallback driver if none exists
                    ...
                    driver.RelationshipGroup = ARMORY_GUARDS;
                ...
                // --- Make driver chase the main driver (in target vehicle) ---
                driver.Task.VehicleChase(mainHeistDriver);
            }

            // Keep the main driver cruising
            if (mainHeistDriver != null && mainHeistDriver.Exists())
            {
                mainHeistDriver.BlockPermanentEvents = true;
                mainHeistDriver.Task.ClearAll();
                mainHeistDriver.Task.CruiseWithVehicle(activeHeistVehicle, 20f);
            }

            heistAIActivated = true;
        }
        ...
        private void OnTick(object sender, EventArgs e)
        {
            ...
            if (!heistAIActivated &&
            activeHeistVehicle != null &&
            activeHeistVehicle.Exists())
            {
                float dist = Game.Player.Character.Position.DistanceTo(
                    activeHeistVehicle.Position
                );

                if (dist <= AI_ACTIVATION_DISTANCE)
                {
                    ActivateHeistAI();
                    heistAIActivated = true;
                }
            }
            foreach (var kvp in pedBlips.ToList())
            {
                Ped ped = kvp.Key;
                Blip blip = kvp.Value;
            ...
            if (activeHeistVehicle != null && activeHeistVehicle.Exists() && activeHeistVehicle.IsDead)
            {
                ...
                if (vehicleDestroyedTime == null)
                {
                    ...
                    // 1. Delete all Ground Troops
                    foreach (Ped ped in npcContactPeds)
                    {
                        if (ped.Exists())
                        {
                            // Remove the blip first
                            if (pedBlips.ContainsKey(ped))
                            {
                                pedBlips[ped].Delete();
                                pedBlips.Remove(ped);
                            }

                            ped.MarkAsNoLongerNeeded(); // Let engine clean up if far away
                            ped.Delete(); // Force immediate deletion
                        }
                    }
                    npcContactPeds.Clear(); // Empty the list for the next mission
            ...
            if (isNearArmoryZone && nearestVehicle != null && Game.Player.Character.IsOnFoot)
            {
                ...
                {
                    string enableKey = GetHelpTextKeyPlaceholder(enable);
                    string heistKeyPlaceholder = GetHelpTextKeyPlaceholder(heistKey);

                    GTA.UI.Screen.ShowHelpText(
                        $"~w~Press ~b~{enableKey}~w~ to buy or modify weapons, or press ~b~{heistKeyPlaceholder}~w~ to rob the vehicle.",
                        0
                    );
                }
            }
            ...
            !pool.AreAnyVisible)
            {
                if (Game.IsControlJustPressed(GTA.Control.PhoneRight) ||
                    Game.IsKeyPressed(enable))
                {
                    HandleArmoryAction();
                }

                if (Game.IsControlJustPressed(GTA.Control.PhoneLeft) ||
                    Game.IsKeyPressed(heistKey))
                {
                    HandleHeistAction();
                }
            }
            if (isNearArmoryZone && (nearestVehicle == null || !nearestVehicle.Exists()) && Game.Player.Character.IsOnFoot)
            {
                ...
                if (currentZone != null && !AnyHeistActive())
                {
                    string enableKey = GetHelpTextKeyPlaceholder(enable);
                    GTA.UI.Screen.ShowHelpText(
                        $"~w~Press ~b~{enableKey}~w~ to buy or modify weapons.",
                        0
                    );
                }
            }
            ...
            foreach (var pair in heistActive.ToList())
            {
                ...
                Vehicle playerVehicle = robberyHeistVehicle;
        ...
        private void HandleArmoryAction()
        {
            if (AnyHeistActive())
            {
                GTA.UI.Notification.Show(
        ...
        private void HandleHeistAction()
        {
            if (AnyHeistActive())
            {
                GTA.UI.Notification.Show(
        ...
        private string GetHelpTextKeyPlaceholder(Keys key)
        {
            // Map only the keys you want to show the icon for
            switch (key)
            {
                case Keys.E: return "~INPUT_CONTEXT~";  // Interact
                case Keys.Q: return "~INPUT_CONTEXT_SECONDARY~";
                default: return key.ToString();     // All other keys show their literal name
            }
        }
        ...
        public Main()
        {
            ...
            iFruitContact contactAgentSteele = new iFruitContact("Agent 16")
            {
            ...
            List<WeaponComponentHash> chcustomComponentHashes = new List<WeaponComponentHash>
                {
                    (WeaponComponentHash)4007263587u,
                    (WeaponComponentHash)4081463091u,
                    (WeaponComponentHash)2539772380u,
                    (WeaponComponentHash)1351683121u,
                    (WeaponComponentHash)2112683568u,
                    (WeaponComponentHash)3800804335u,
                    (WeaponComponentHash)1062111910u,
                    (WeaponComponentHash)3323197061u,
                    (WeaponComponentHash)146278587u,
                    (WeaponComponentHash)2062808965u,
                    (WeaponComponentHash)2436343040u,
                    (WeaponComponentHash)1530822070u,
                    (WeaponComponentHash)3885209186u,
                    (WeaponComponentHash)3372082259u,
                    (WeaponComponentHash)3014965697u


                };
            armoryMenu.Alignment = Alignment.Left;
            ...
            pool.Add(cHWeapon);
            cHWeapon.Alignment = Alignment.Left;
            ...
            cHWeapon.Shown += (sender, args) =>
            {
                ...
                if (!WeaponValues.ContainsKey(heldWeaponHash))
                {
                    ...
                    GTA.UI.Notification.Show(
                        ...
                        "Ammu-Nation +",
                        "Important",
                        $"~r~You are not holding a supported weapon.",
                        true,
                        false
                    );
                    return;
                ...
                string name = heldWeaponHash.ToString();
                ...
                buyAmmoItem.Activated += (s, a) =>
                {
                    ...
                    if (!weapons.HasWeapon(heldWeaponHash))
                    {
                        GTA.UI.Notification.Show(
                            ...
                            "Ammu-Nation +",
                            "Important",
                            $"~w~You don't own the ~b~{name}. ~w~Buy the weapon first.",
                            true,
                            false
                        );
                        return;
                    ...
                    if (Game.Player.Money < ammoPrice)
                    {
                        GTA.UI.Notification.Show(
                            ...
                            "Ammu-Nation +",
                            "Important",
                            $"~r~Not enough money! You need ~b~${ammoPrice}",
                            true,
                            false
                        );
                    ...
                    GTA.UI.Notification.Show(
                            ...
                            "Ammu-Nation +",
                            "Important",
                            $"~w~You purchased ~b~{ammoAmount}~w~ rounds for ~r~-${ammoPrice}",
                            true,
                            false
                        );
                ...
                buyFullAmmoItem.Activated += (s, a) =>
                {
                    ...
                    if (!weapons.HasWeapon(heldWeaponHash))
                    {
                        GTA.UI.Notification.Show(
                            ...
                            "Ammu-Nation +",
                            "Important",
                            $"~r~You don't own the ~b~{name}. ~w~Buy the weapon first.",
                            true,
                            false
                        );
                        return;
                    ...
                    if (Game.Player.Money < refillPrice)
                    {
                        GTA.UI.Notification.Show(
                            ...
                            "Ammu-Nation +",
                            "Important",
                            $"~r~Not enough money!~w~ You need ~b~${refillPrice}",
                            true,
                            false
                        );
                        return;
                    ...
                    GTA.UI.Notification.Show(
                            ...
                            "Ammu-Nation +",
                            "Important",
                            $"~w~Fully refilled ammo for ~r~${refillPrice}",
                            true,
                            false
                        );
                };
                ...
                sellItem.Activated += (s1, a1) =>
                {
                    ...
                    GTA.UI.Notification.Show(GTA.UI.NotificationIcon.Ammunation, "Ammu-Nation +", "Alert", $"~w~Sold~b~ {name} for ~g~${price}", true, false);
                ...
                customizeMenu.Alignment = Alignment.Left;
                // START HERE
                ...
                customizeMenu.Shown += (s, e) =>
                {
                    if (!weapons.HasWeapon(heldWeaponHash))
                    {
                        ...
                        GTA.UI.Notification.Show(GTA.UI.NotificationIcon.Ammunation, "Ammu-Nation +", "Alert", $"~w~You don't own the ~b~{name}.", true, false);
                        return;
                    }
            ...
            {
                selected.RightBadge = CreateBadge();
                other.RightBadge = null;
            }
            ...
            meleeSubMenu.Alignment = Alignment.Left;
            ...
            foreach (var melee in MeleeValues)
            {
                ...
                // Create the weapon menu (submenu)
                ...
                pool.Add(weaponMenu);
                weaponMenu.Alignment = Alignment.Left;
                ...
                equipItem.Activated += (s, a) =>
                {
                    ...
                    GTA.UI.Notification.Show(GTA.UI.NotificationIcon.Ammunation, "Ammu-Nation +", "Alert", $"~w~Equipped~b~ {name}", true, false);
                ...
                customizeMenu.Shown += (s, e) =>
                {
                    if (!Game.Player.Character.Weapons.HasWeapon(hash))
                    {
                        ...
                        GTA.UI.Notification.Show(
                            ...
                            "Ammu-Nation +",
                            "Important",
                            $"~r~You no longer own the ~b~{name}.",
                            true,
                            false
                        );
                ...
                buyItem.Activated += (s, a) =>
                {
                    ...
                    if (weapons.HasWeapon(hash))
                    {
                        GTA.UI.Notification.Show(GTA.UI.NotificationIcon.Ammunation, "Ammu-Nation +", "Alert", $"~w~You already own the ~b~{name}.", true, false);
                        return;
                    }

                    if (Game.Player.Money < price)
                    {
                        GTA.UI.Notification.Show(GTA.UI.NotificationIcon.Ammunation, "Ammu-Nation +", "Insufficient Funds", $"~r~Not enough money!~w~ You need ~b~${price}", true, false);
                        return;
                    }

                    Game.Player.Money -= price;
                    weapons.Give(hash, 1, true, true);
                    GTA.UI.Notification.Show(GTA.UI.NotificationIcon.Ammunation, "Ammu-Nation +", "Receipt", $"~w~You purchased ~g~{name}", true, false);
                ...
                sellItem.Activated += (s, a) =>
                {
                    ...
                    weapons.Remove(hash);
                    Game.Player.Money += price;
                    GTA.UI.Notification.Show(GTA.UI.NotificationIcon.Ammunation, "Ammu-Nation +", "Receipt", $"~w~Sold {name} for ~g~${price}", true, false);
                ...
                // Compute stats
                int damage = (int)(Function.Call<float>(Hash.GET_WEAPON_DAMAGE, (uint)hash) * 10f);
                int speed = (int)(100f - Function.Call<float>(Hash.GET_WEAPON_TIME_BETWEEN_SHOTS, (uint)hash) * 100f);
                int accuracy = (int)Function.Call<float>(Hash.GET_PED_ACCURACY, Game.Player.Character.Handle);
                int range = 50; // placeholder

                var damageStat = new NativeStatsInfo("Damage", damage);
                var speedStat = new NativeStatsInfo("Fire Rate", speed);
                var accuracyStat = new NativeStatsInfo("Accuracy", accuracy);
                var rangeStat = new NativeStatsInfo("Range", range);

                var statsPanel = new NativeStatsPanel(damageStat, speedStat, accuracyStat, rangeStat);

                // Call the panel on these items
                buyItem.Panel = statsPanel;
                equipItem.Panel = statsPanel;
                sellItem.Panel = statsPanel;
                sellItem.Panel = statsPanel;
            ...
            handgunsSubMenu.Alignment = Alignment.Left;
            ...
            foreach (var handgun in HandgunsValues)
            {
                ...
                buyAmmoItem.Activated += (s, a) =>
                {
                    ...
                    if (!weapons.HasWeapon(hash))
                    {
                        GTA.UI.Notification.Show(GTA.UI.NotificationIcon.Ammunation, "Ammu-Nation +", "Alert", $"~r~You don't own the {name}. ~w~Buy the weapon first.", true, false);
                        return;
                    }
                    int ammoPrice = 70;
                    int ammoAmount = 50;
                    ...
                    if (Game.Player.Money < ammoPrice)
                    {
                        GTA.UI.Notification.Show(GTA.UI.NotificationIcon.Ammunation, "Ammu-Nation +", "Insufficient Funds", $"~r~Not enough money!~w~ You need ~b~${ammoPrice}", true, false);
                        return;
                    }
                    ...
                    GTA.UI.Notification.Show(GTA.UI.NotificationIcon.Ammunation, "Ammu-Nation +", "Receipt", $"~w~Purchased~b~ {ammoAmount} ~w~rounds for ~r~${ammoPrice}", true, false);
                };
                ...
                buyFullAmmoItem.Activated += (s, a) =>
                {
                    ...
                    if (Game.Player.Money < refillPrice)
                    {
                        GTA.UI.Notification.Show(GTA.UI.NotificationIcon.Ammunation, "Ammu-Nation +", "Insufficient Funds", $"~r~Not enough money! You need ${refillPrice}", true, false);
                        return;
                    }
                    ...
                    GTA.UI.Notification.Show(GTA.UI.NotificationIcon.Ammunation, "Ammu-Nation +", "Receipt", $"~w~Fully refilled ammo for ~r~${refillPrice}", true, false);
                };
                ...
                customizeMenu.Shown += (s, e) =>
                {
                    if (!Game.Player.Character.Weapons.HasWeapon(hash))
                    {
                        ...
                        GTA.UI.Notification.Show(GTA.UI.NotificationIcon.Ammunation, "Ammu-Nation +", "Important", $"~r~You no longer own the {name}.", true, false);
                    }
                };
                ...
                openCompMenuItem.Activated += (sender1, args1) =>
                {
                    ...
                    if (!weapon.IsPresent)
                    {
                        GTA.UI.Notification.Show(GTA.UI.NotificationIcon.Ammunation, "Ammu-Nation +", "Alert", $"~w~You need to own the ~b~{name} to customize it.", true, false);
                        return;
                    }
            ...
            pool.Add(smgSubMenu);
            smgSubMenu.Alignment = Alignment.Left;
    ...
    "Default / Black", "Green", "Gold", "Pink", "Army", "LSPD", "Orange", "Platinum"
        ...
        "Default", "Color 1", "Color 2", "Color 3", "Color 4", "Color 5", "Color 6", "Color 7"
            ...
            riflesSubMenu.Alignment = Alignment.Left;
        ...
        "Default", "Color 1", "Color 2", "Color 3", "Color 4", "Color 5", "Color 6", "Color 7"
            ...
            machinegunsSubMenu.Alignment = Alignment.Left;
            ...
            shotgunsSubMenu.Alignment = Alignment.Left;
            ...
            snipersSubMenu.Alignment = Alignment.Left;
            ...
            foreach (var handgun in SnipersValues)
            {
                ...
                buyItem.Activated += (s, a) =>
                {
                    ...
                    if (Game.Player.Money < price)
                    {
                        GTA.UI.Notification.Show(GTA.UI.NotificationIcon.Ammunation, "Ammu-Nation +", "Important", $"~r~Not enough money! ~w~You need ~b~${price}", true, false);
            ...
            heavyweaponSubMenu.Alignment = Alignment.Left;
            ...
            heavyweaponSubMenu.Shown += (sender, args) =>
            {
                ...
                foreach (var pair in heavyweaponItems)
                {
                    ...
                    // Assuming you have a dictionary of equipItems for handguns:
                    var equipItem = heavyweaponEquipItems[hash];
            ...
            throwableSubMenu.Alignment = Alignment.Left;
            ...
            throwableSubMenu.Shown += (sender, args) =>
            {
                ...
                foreach (var pair in throwableItems)
                {
                    ...
                    var equipItem = throwableEquipItems[hash];
        ...
        private BlipSprite GetBlipSprite(string spriteString)
        {
            ...
            // Try parsing as integer code first
            if (int.TryParse(spriteString, out int code))
            {
                if (Enum.IsDefined(typeof(BlipSprite), code))
                    return (BlipSprite)code;
                return BlipSprite.Standard;
            }

            // Try parsing as enum name (case-insensitive)
            if (Enum.TryParse<BlipSprite>(spriteString, true, out var sprite))
                return sprite;
            ...
            try
            {
                ...
                if (Game.Player.Money < totalCost)
                {
                    GTA.UI.Notification.Show(
                        ...
                        "Ammu-Nation +",
                        "Ammu-Nation +",
                        $"~r~Not enough money! ~w~You need ~y~${totalCost}",
                        true,
                        false
                    );
                    return;
                ...
                {
                    weapons.Give(hash, 999, true, false);
                ...
                GTA.UI.Notification.Show(
                    ...
                    "Ammu-Nation +",
                    "Ammu-Nation +",
                    $"Purchased all weapons for ~r~ -${totalCost}",
                    true,
                    false
                );
                return;
        ...
        private void RemoveWeapons()
        {
            ...
            try
            {
                ...
                GTA.UI.Notification.Show(
                    ...
                    "Ammu-Nation +",
                    "Alert",
                    $"~w~Sold all weapons for $~g~{totalValue}",
                    true,
                    false
                );
